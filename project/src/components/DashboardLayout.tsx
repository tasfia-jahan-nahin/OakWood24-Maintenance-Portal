  ...
  return (
    <div className="min-h-screen bg-pink-50 flex">
      ...
      <aside className="hidden lg:flex w-64 flex-col bg-white/70 backdrop-blur-md border-r border-pink-100 fixed inset-y-0 left-0 z-30">
        <SidebarContent current={current} onNavigate={handleNav} user={user?.email} onSignOut={signOut} />
      </aside>
      ...
      {mobileOpen && (
        <>
          ...
          <aside className="lg:hidden fixed inset-y-0 left-0 z-50 w-64 bg-white flex flex-col animate-slide-in-right">
            <SidebarContent current={current} onNavigate={handleNav} user={user?.email} onSignOut={signOut} onClose={() => setMobileOpen(false)} />
          </aside>
        </>
      )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="hidden sm:block">
                <p className="text-xs font-semibold text-pink-800 leading-tight">{user?.email ?? 'User'}</p>
                <p className="text-xs text-pink-400 leading-tight">Administrator</p>
              </div>
            </div>
          </div>
        </header>
